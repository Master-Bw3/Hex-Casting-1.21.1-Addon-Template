package io.github.example.hextemplate.registry
...
typealias RegistrarEntry<T> = HexTemplateRegistrar<T>.Entry<out T>

abstract class HexTemplateRegistrar<T : Any>(
    ...
    fun <V : T> register(name: String, builder: () -> V): Entry<V> = register(HexTemplate.id(name), builder)
    ...
    open inner class Entry<V : T>(
        ...
        override fun equals(other: Any?) = when (other) {
            is HexTemplateRegistrar<*>.Entry<*> -> key.registry().equals(other.key.registry()) && id.equals(other.id)
            else -> false
        }
